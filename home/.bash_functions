...
# herdr's config.toml (wrapped in `# chs:start`/`# chs:end` markers,
# grouped there by level with navigation last), so both the bindings and
# their grouping can never drift out of sync with it; kill_all_workspaces
# lives outside that table as a [[keys.command]] block, so it's slotted
# in here right after close_workspace (its own group) and needs updating
# manually if it ever changes in either config.
mux-keys() {
    ...
    awk '
        ...
        f {
            print
            if ($0 ~ /^close_workspace/) print "kill_all_workspaces = \"prefix+shift+k\""
        }
